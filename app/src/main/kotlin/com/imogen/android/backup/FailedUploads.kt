 ...
 *
 * The ledger keeps every row it has ever written, across every key; a pass only ever
 * visits the accounts in the book. So rows left behind by an account that has since been
 * signed out — the default, because `forgetBackups` is off and those rows are what lets
 * signing back in resume instead of re-uploading the roll — name a destination nothing
 * owns any more. Nothing retries them, "Try again" cannot reach them, and they sat on the
 * screen inflating the count for ever. Rows an old version keyed on a device-local
 * account id are the same case: `adoptStableKeys` only knows accounts still in the book,
 * so those are unreachable too, and render the bare UUID where the server's name goes.
 *
 * Hidden rather than deleted. The account may well come back, and its rows are correct
 * again the moment it does.
 */
fun reachable(failures: List<FailedUpload>, backupKeys: Set<String>): List<FailedUpload> =
    failures.filter { it.backupKey in backupKeys }
